    ...
    def __init__(self, id, server_addr_0=SERVER_ADDR_0, server_addr_1=SERVER_ADDR_1,
        ...
        if self.id == 0:
            ...
            self.listen_wrapper([self.server_facing_sockets[0], self.client_facing_socket], 
                                [self.server_connection_ports[2], self.ports[self.id]])
            self.server_facing_sockets[1].connect((self.addresses[0], self.server_connection_ports[0]))
        elif self.id == 2:
            self.listen_wrapper([self.client_facing_socket], [self.ports[self.id]])
            self.server_facing_sockets[0].connect((self.addresses[0], self.server_connection_ports[1]))
            self.server_facing_sockets[1].connect((self.addresses[1], self.server_connection_ports[2]))
        else:
